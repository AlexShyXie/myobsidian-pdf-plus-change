...
export class PDFPlusCommands extends PDFPlusLibSubmodule {
    ...
    createPDF() {
        ...
        new PDFCreateModal(this.plugin)
            .askOptions()
            .then(async (doc) => {
                const file = await this.app.vault.createBinary(path, (await doc.save()).buffer as ArrayBuffer);
        ...
        if (!checking) {
            new PDFOutlineTitleModal(this.plugin, 'Add to outline')
                ...
                .then(async ({ title }) => {
                    ...
                    await this.app.vault.modifyBinary(file, (await doc.save()).buffer as ArrayBuffer);
                });
        }
