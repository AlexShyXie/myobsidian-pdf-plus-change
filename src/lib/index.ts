...
export class PDFPlusLib {
    ...
    async getPdfLibDocument(activeOnly: boolean = false) {
        ...
        if (doc) {
            return await this.loadPdfLibDocumentFromArrayBuffer((await doc.getData()).buffer as ArrayBuffer);
        }
    }
    ...
    async getPdfLibPage(activeOnly: boolean = false) {
        ...
        const doc = await this.loadPdfLibDocumentFromArrayBuffer((await pdfViewer.pdfDocument.getData()).buffer as ArrayBuffer);
        if (doc) {
    ...
    async write(path: string, data: string | ArrayBuffer, existOk: boolean): Promise<TFile | null> {
        ...
        if (file instanceof TFile) {
            ...
            if (typeof data === 'string') {
                ...
                return await this.app.vault.createBinary(path, data as ArrayBuffer);
            }
        }
